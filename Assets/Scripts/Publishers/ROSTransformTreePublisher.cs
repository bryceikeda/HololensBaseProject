...
using Unity.Robotics.TransformTree;
...
public class ROSTransformTreePublisher : MonoBehaviour
{
    ...
    public GameObject RootGameObject { get => m_RootGameObject; set => m_RootGameObject = value; }
    ...
    void PublishMessage()
    {
        ...
        if (m_GlobalFrameIds.Count > 0)
        {
            var tfRootToGlobal = new TransformStampedMsg(
                new HeaderMsg(0, new TimeStamp(Clock.time), m_GlobalFrameIds.Last().ToString()),
